...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private static int leftIndex(int i) {
        if (i == 0) {
            throw new java.lang.IllegalArgumentException("i can't be 0");
        } else {
            return 2 * i;
        }
    }
    ...
    private static int parentIndex(int i) {
        if (i == 1) {
            throw new java.lang.IllegalArgumentException(" no parent");
        } else {
            return i / 2;
        }
    }
     ...
     */
    private boolean greater(int leaf, int root) {
        return contents[leaf].myPriority >= contents[root].myPriority;
    }
